...
def select_resnet(network, norm="bn"):
    # Keep a single param dict; callers rely on feature_size later (e.g., head dims)
    param = {"feature_size": 1024}

    if network == "resnet18":
        # UNCHANGED: 18 already returned four modality-specific backbones.
        # img = RGB (3ch), seg/depth = single channel, flow = 2 channels
        model_img = resnet18_2d3d_full(track_running_stats=True, norm=norm)
        model_flow = resnet18_2d3d_full_C2(track_running_stats=True, norm=norm)
        model_seg  = resnet18_2d3d_full_C1(track_running_stats=True, norm=norm)
        model_depth= resnet18_2d3d_full_C1(track_running_stats=True, norm=norm)

        # CHANGED: make feature size reflect BasicBlock (expansion=1 → 256).
        param["feature_size"] = 256

    elif network == "resnet34":
        # CHANGED: previously only built a single `model` and then returned four names,
        # which would crash. Now we mirror the resnet18 branch with four backbones.
        model_img = ResNet2d3d_full(
            [BasicBlock2d, BasicBlock2d, BasicBlock3d, BasicBlock3d],
            [3, 4, 6, 3],
            track_running_stats=True, norm=norm
        )
        model_flow = ResNet2d3d_full_C2(
            [BasicBlock2d, BasicBlock2d, BasicBlock3d, BasicBlock3d],
            [3, 4, 6, 3],
            track_running_stats=True, norm=norm
        )
        model_seg = ResNet2d3d_full_C1(
            [BasicBlock2d, BasicBlock2d, BasicBlock3d, BasicBlock3d],
            [3, 4, 6, 3],
            track_running_stats=True, norm=norm
        )
        model_depth = ResNet2d3d_full_C1(
            [BasicBlock2d, BasicBlock2d, BasicBlock3d, BasicBlock3d],
            [3, 4, 6, 3],
            track_running_stats=True, norm=norm
        )
        # CHANGED: like resnet18 (BasicBlock), feature maps end at 256.
        param["feature_size"] = 256

    elif network == "resnet50":
        # CHANGED: same reason—return four backbones consistently.
        model_img = ResNet2d3d_full(
        ...
        # CHANGED: Bottleneck (expansion=4) → 1024 final feature channels.
        param["feature_size"] = 1024

    else:
        # CHANGED: make the error message actionable for debugging.
        raise NotImplementedError(f"Unsupported network '{network}'")

    # CHANGED: always return the same tuple shape in the same order,
    # so callers can rely on it regardless of network.
    return model_img, model_seg, model_depth, model_flow, param
...
def resnet18(pretrained=False, progress=True, **kwargs):  # ADDED: public alias expected by callers
    return resnet18_custom(pretrained=pretrained, progress=progress, **kwargs)
...
class Bottleneck3d(nn.Module):
    ...
    def __init__(
        ...
        if norm == "bn":
            ...
            self.n1 = nn.Identity()  # FIXED: was 'self.n2 = nn.Identity()' (wrong target)  # CHANGED
