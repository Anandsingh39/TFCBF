...
key = jr.PRNGKey(0)
graph = env.reset(key)
print("Initial EnvState:", graph.env_states)
print("Agent nodes:", graph.type_states(type_idx=DoubleIntegrator.AGENT, n_type=env.num_agents))
print("Goal nodes:", graph.type_states(type_idx=DoubleIntegrator.GOAL, n_type=env.num_agents))

action = jax.numpy.zeros((env.num_agents, env.action_dim))
next_graph, reward, cost, done, info = env.step(graph, action, get_eval_info=True)
print("Next EnvState:", next_graph.env_states)
print("Reward:", reward, "Cost:", cost, "Done:", done)
print("Info:", info)