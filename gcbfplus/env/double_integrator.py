...
class DoubleIntegrator(MultiAgentEnv):
    ...
    def safe_mask(self, graph: GraphsTuple) -> Array:
        ...
        if self.num_agents == 1:
            # "safe" = outside obstacles with a clearance of 2*car_radius
    ...
    def unsafe_mask(self, graph: GraphsTuple) -> Array:
        ...
        # ---- single-agent fast path ----
        if self.num_agents == 1:
            # immediate obstacle collision contact
            unsafe_obs = inside_obstacles(agent_pos, graph.env_states.obstacle, self._params["car_radius"])
            # heading-based "unsafe direction" using LiDAR pseudo-nodes
            n_hits = self._params["n_rays"] * self.num_agents  # = n_rays
            obs_pos = graph.type_states(type_idx=2, n_type=n_hits)[:, :2]  # [n_rays, 2]
            obs_pos_diff = obs_pos[None, :, :] - agent_pos[:, None, :]     # [1, n_rays, 2]
            obs_dist = jnp.linalg.norm(obs_pos_diff, axis=-1)              # [1, n_rays]
    
            # warn zones
            obs_warn_dist = 2 * self._params["car_radius"]
            warn_zone_obs = jnp.less(obs_dist, obs_warn_dist)              # [1, n_rays]
    
            # direction alignment with current heading
            pos_vec = obs_pos_diff / (jnp.linalg.norm(obs_pos_diff, axis=2, keepdims=True) + 1e-4)  # [1, n_rays, 2]
            speed_agent = jnp.linalg.norm(agent_state[:, 2:], axis=1, keepdims=True)                # [1, 1]
            heading_vec0 = (agent_state[:, 2:] / (speed_agent + 1e-4))[:, None, :]                  # [1, 1, 2]
            heading_vec = jnp.repeat(heading_vec0, pos_vec.shape[1], axis=1)                        # [1, n_rays, 2]
            inner_prod = jnp.sum(pos_vec * heading_vec, axis=2)                                     # [1, n_rays]
    
            # angle threshold for obstacle "cone"
            unsafe_theta_obs = jnp.arctan2(self._params["car_radius"],
                                           jnp.sqrt(jnp.maximum(obs_dist**2 - self._params["car_radius"]**2, 1e-9)))
            unsafe_dir = jnp.max(jnp.logical_and(warn_zone_obs, jnp.greater(inner_prod, jnp.cos(unsafe_theta_obs))), axis=1)  # [1]
    ...
    def finish_mask(self, graph: GraphsTuple) -> Array:
        ...
        return reach  # works for single-agent (shape (1,)) and multi-agent
